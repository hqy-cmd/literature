...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"PingFang SC","Noto Sans SC","Helvetica Neue",Arial,sans-serif; background:var(--bg); color:var(--text); line-height:1.6; }
    ...
    .tag { background:var(--tag); color:#314155; border-radius:999px; padding:4px 10px; font-size:12px; } .actions { display:flex; flex-wrap:wrap; gap:8px; margin:8px 0 2px; } .link-btn { display:inline-flex; align-items:center; border:1px solid var(--line); border-radius:10px; padding:8px 12px; text-decoration:none; color:var(--accent); background:#fff; font-size:14px; font-weight:700; cursor:pointer; }
    ...
    .editor-backdrop { position:fixed; inset:0; background:rgba(15,23,42,.35); display:none; align-items:flex-end; justify-content:flex-end; z-index:999; }
    .editor-backdrop.open { display:flex; }
    .editor-panel { width:min(560px, 100vw); height:100vh; background:#fff; box-shadow:-12px 0 32px rgba(15,23,42,.18); padding:18px; overflow:auto; }
    ...
    .editor-grid label { display:grid; gap:6px; font-size:13px; color:var(--muted); }
    .editor-grid input, .editor-grid textarea, .editor-grid select { width:100%; border:1px solid var(--line); border-radius:10px; padding:10px 12px; font-size:14px; font-family:inherit; color:var(--text); background:#fff; }
    .editor-grid textarea { min-height:110px; resize:vertical; }
    .editor-actions { display:flex; gap:10px; margin-top:14px; }
    .btn-primary { background:var(--accent); color:#fff; border-color:var(--accent); }
    .editor-status { margin-top:10px; font-size:13px; color:var(--muted); }
...
<body>
  ...
  <div id="editorBackdrop" class="editor-backdrop">
    <div class="editor-panel">
      <div class="editor-head">
        <div class="editor-title">编辑文献</div>
        <button type="button" class="link-btn" id="editorCloseBtn">关闭</button>
      </div>
      <div class="editor-grid">
        <label>标题<input id="editTitle" /></label>
        <label>作者（逗号分隔）<textarea id="editAuthors"></textarea></label>
        <label>年份<input id="editYear" /></label>
        <label>一级分类<select id="editCategory"></select></label>
        <label>分类集合（逗号分隔）<textarea id="editCollections"></textarea></label>
        <label>标签（逗号分隔）<textarea id="editTags"></textarea></label>
        <label>中文摘要<textarea id="editSummary"></textarea></label>
        <label>来源备注<textarea id="editSourceNote"></textarea></label>
      </div>
      <div class="editor-actions">
        <button type="button" class="link-btn btn-primary" id="editorSaveBtn">保存</button>
        <button type="button" class="link-btn" id="editorCancelBtn">取消</button>
      </div>
  ...
  <script>
    ...
    const el = { consoleInput:document.getElementById('consoleInput'), resultBar:document.getElementById('resultBar'), resultSummary:document.getElementById('resultSummary'), clearSmartSearchBtn:document.getElementById('clearSmartSearchBtn'), paperList:document.getElementById('paperList'), menuTree:document.getElementById('menuTree'), paperCount:document.getElementById('paperCount'), collectionCount:document.getElementById('collectionCount'), editorBackdrop:document.getElementById('editorBackdrop'), editorCloseBtn:document.getElementById('editorCloseBtn'), editorCancelBtn:document.getElementById('editorCancelBtn'), editorSaveBtn:document.getElementById('editorSaveBtn'), editorStatus:document.getElementById('editorStatus'), editTitle:document.getElementById('editTitle'), editAuthors:document.getElementById('editAuthors'), editYear:document.getElementById('editYear'), editCategory:document.getElementById('editCategory'), editCollections:document.getElementById('editCollections'), editTags:document.getElementById('editTags'), editSummary:document.getElementById('editSummary'), editSourceNote:document.getElementById('editSourceNote') };
    ...
    function tokenizeQuery(text) {
      ...
      const phrases = [];
      const knownPhrases = ['机器人识别', '机器人抓取', '光声显微', '纳米温度计', '温度成像', '脑肿瘤', '脑组织', '肿瘤消融', '灵巧手', '视觉触觉融合', '多智能体强化学习'];
      knownPhrases.forEach(p => { if (raw.includes(p)) phrases.push(p); });
      const zhChunks = raw.match(/[\u4e00-\u9fff]{2,}/g) || [];
      const zhSingles = [...new Set(zhChunks.flatMap(x => x.length <= 4 ? [x] : x.split(/(?<=..)/).filter(Boolean)).filter(x => x.length >= 2))];
      const enWords = raw.match(/[a-zA-Z][a-zA-Z\-]+/g) || [];
      const stop = new Set(['找一下','相关文章','关于','哪些','有关','方面','研究','一下','相关','文献','文章','创新点']);
      const base = [...phrases, ...zhSingles, ...enWords].filter(x => x && !stop.has(x));
      const expansions = [];
      const map = [
        { keys: ['机器人识别', '机器人抓取', '灵巧手', '抓取'], add: ['robotic hand', 'grasp', 'grasping', 'dexterous hand', '机器人抓取', '灵巧手'] },
        { keys: ['识别', '视觉', '触觉'], add: ['visual', 'tactile', '视觉触觉融合'] },
        { keys: ['光声显微', '光声'], add: ['photoacoustic microscopy', 'photoacoustic', '光声显微'] },
        { keys: ['纳米温度计', '温度成像', '测温'], add: ['nanothermometer', 'temperature imaging', 'thermometry', '荧光纳米温度计'] },
        { keys: ['脑肿瘤', '胶质瘤'], add: ['glioma', 'glioblastoma', 'brain tumor', '脑肿瘤'] },
        { keys: ['消融', '热疗'], add: ['tumor ablation', 'thermal ablation', 'microwave ablation', '肿瘤消融'] },
      ];
    ...
    function scorePaperByTokens(paper, tokens) {
      ...
      for (const token of tokens) {
        if (!token) continue;
        let hit = false;
        const isPhrase = token.length >= 4 || token.includes(' ');
        if (title.includes(token)) { score += isPhrase ? 12 : 8; hit = true; strongHits += 1; }
        if (tags.includes(token)) { score += isPhrase ? 8 : 6; hit = true; }
        if (category.includes(token) || collections.includes(token)) { score += isPhrase ? 7 : 5; hit = true; }
        if (authors.includes(token)) { score += 4; hit = true; }
        if (abs.includes(token)) { score += isPhrase ? 4 : 2; hit = true; }
        if (filename.includes(token)) { score += 1; hit = true; }
        if (hit) matched.push(token);
      }
      const uniqueMatched = [...new Set(matched)];
      if (strongHits === 0 && uniqueMatched.length < 2) score = 0;
      if (uniqueMatched.length >= 3) score += 4;
    ...
    function runSmartSearch(query) {
      ...
      }).filter(x => x._score > 0).sort((a, b) => b._score - a._score).slice(0, 6);
    ...
    function openEditor(id) {
      ...
      el.editAuthors.value = normalizeList(paper.authors).join(', ');
      el.editYear.value = paper.year || '';
      fillCategoryOptions(paper.category || '其他');
      el.editCollections.value = normalizeList(paper.collections).join(', ');
      el.editTags.value = normalizeList(paper.tags).join(', ');
      el.editSummary.value = paper.abstract_summary_zh || '';
      el.editSourceNote.value = paper.source_note || '';
    ...
    async function saveEditor() {
      ...
      const payload = {
        ...
        updates: {
          ...
          authors: el.editAuthors.value,
          year: el.editYear.value,
          category: el.editCategory.value,
          collections: el.editCollections.value,
          tags: el.editTags.value,
          abstract_summary_zh: el.editSummary.value,
          source_note: el.editSourceNote.value || '网页人工校正',
        }
      };
      try {
        const resp = await fetch('http://127.0.0.1:8766/api/papers/update', {
          method: 'POST',
        ...
        if (!resp.ok || !result.ok) {
          throw new Error((result && (result.error || (result.rebuild && result.rebuild.stderr))) || '保存失败');
        }
        const paper = state.allPapers.find(p => p.id === state.editingId);
        if (paper) {
          paper.title = el.editTitle.value.trim();
          paper.authors = normalizeList(el.editAuthors.value);
          paper.year = el.editYear.value.trim();
          paper.category = el.editCategory.value.trim();
          paper.collections = normalizeList(el.editCollections.value);
          paper.tags = normalizeList(el.editTags.value);
          paper.abstract_summary_zh = el.editSummary.value.trim();
          paper.source_note = (el.editSourceNote.value || '网页人工校正').trim();
        }
        renderMenu();
        renderPapers();
        el.editorStatus.textContent = '保存成功，已同步。';
        setTimeout(() => closeEditor(), 500);
      } catch (err) {
        el.editorStatus.textContent = `保存失败：${err.message}`;
      }
    }
    function renderMenu() {
    ...
    el.editorBackdrop.addEventListener('click', e => { if (e.target === el.editorBackdrop) closeEditor(); });
    renderMenu(); renderPapers();
    el.paperList.addEventListener('click', e => {
